...
Initialised with `git init -b main`. The folder on disk is 181 MB; the
repo is 11 MB. The difference is deliberate — commit the source and the
pipeline, not the raw material it was made from nor what it produces.

### Not tracked, and why

| Excluded | Size | Why it is safe to leave out |
|---|---|---|
| `doc/*.pdf`, `doc/pages/`, `doc/menu-avocado.jpg` | 80 MB | The restaurant own menu PDFs and the page scans cut out of them. Every price already lives in `data/menu.json` and every dish photo was already cropped into `img/`. `menu-food.pdf` alone is 35 MB. |
| `_qa/shots/`, `_qa/*.png` | 66 MB | Headless Edge screenshots. Regenerated by `_qa\shot.ps1` and `_qa\mshot.ps1`, and they churn on every run. |
| `img/raw/`, `img/tiktok-*/` | 14 MB | Full-res Facebook originals and the TikTok frame dump — the input to `_qa\extract-venue.ps1`. The web-sized JPEGs it produced are tracked. |
| `rabbit-grill-artifact.html` | 8.5 MB | Build output. Derived entirely from the tracked pages, CSS, JS and `img/`. |

`doc/RESEARCH.md` **is** tracked — it is the provenance record for every
published fact, and it is the reason nothing on this site is invented.

Keep the excluded originals in the client folder. They are the only copies.

### A fresh clone is complete

All 51 images the site references are tracked, so `serve.ps1` runs straight
after cloning, and the artifact is one command away:

    powershell -NoProfile -ExecutionPolicy Bypass -File _qa\build-artifact.ps1

What a clone cannot do is re-run `_qa\extract-images.ps1`, which reads the
untracked menu PDFs. Its outputs are committed, so this only matters if the
crops ever need re-cutting — bring the PDFs back for that.

### Two traps for anyone editing after cloning

`.gitattributes` keeps `*.ps1` as CRLF. Beyond that, **the .ps1 files are
UTF-8 with BOM and must stay that way** — git preserves the BOM, but an
editor that saves without one will mangle every Thai string in the script.
PowerShell 5.1 gives no warning when this happens.

And the images are the restaurant own copyrighted photographs, taken from
their public Facebook page and their menus. That is a reason to think about
who can see this repo before making it public.
